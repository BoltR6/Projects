...
console.time('Init')
const fs = require('fs');
const FULL = fs.readFileSync('data/possible.txt', 'utf8').split('\n');
...
async function wordsLeft(wordList, params) {
	let wordReturn = wordList;
	let keyf = params.yes + "|" + params.no + "|" + params.yes_at + "|" + params.no_at;
	let possible = await all_db.findOne({key:keyf});
	if (!possible.value) {
...
async function scoreWord(word, list, len) {
	let score = 0;
	console.log('')
	process.stdout.write('[')
	for(var i = 0;i < list.length;i++){
		var chars = wordleTest(word.split(''), list[i].split(''));
		let ans = await wordsLeft(FULL, chars);
		score += ans.length / len;
...
database = client.db("wordle");
all_db = database.collection("all");
try {
	let testList = [...FULL.slice(0, 172)];
	for(var i = 0;i < FULLLENGTH;i++){
		console.time('Entry')
		let thing = await scoreWord(FULL[i], FULL, FULLLENGTH)
		console.log(FULL[i] + " (" + (i+1) + "/170 ): " + thing);
